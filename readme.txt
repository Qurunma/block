...
address candidate;
bool idClosed;
mapping (address=>bool) votesOfPeople;
uint countOfVoted;
}
...
require(Users[idUser].role == 0, "You are
not admin");
...
function add_new_admin(address candidate) public isAdmin(candidate) {
mapping (address => bool) isVoted;
isVoted[msg.sender]=true;
Votes.push(vote(Votes.length, candidate, false, isVoted, 1));
}
}